...
> 4.跟上面的完全不同！该命令是`完完全全`的进行`回滚`！`不仅`仅是文件的区状态会发生改变，连`文件的内容`也会`进行回滚`到上次commit的版本的文件内容！

~~~html
git reset --hard HEAD~1
~~~

> ⚠️⚠️⚠️：使用该命令后，当前commit的版本包括当前在工作区的还是暂存区的或者是本地仓库区的文件都会进行回滚到上一个commit版本。就算是使用 `git reflog`命令进行恢复这个版本的时候，恢复到之前的commit版本状态，但是注意这时候之前未在本地仓库区的文件(也就是commit之前的文件)都会`丢失`！！
